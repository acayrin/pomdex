...
export default class LevelGuide {
	...
	constructor(command: string) {
		...
		// assign level array
		this.#guideOptions._levelArray = Utils.range(this.#guideOptions._levelStart, this.#guideOptions._levelEnd + 1);
				...
				[
					(next: AsyncNext) => {
						...
						if (!this.#guideOptions.skipCache && pomieLevelModels[level]) {
							this.#taskGLMMUnzip(level).then((model) => next(null, model));
						} else {
							this.#taskGLMMGenNew(level).then((model) => next(null, model));
						}
					},
	...
	#taskGLMMGenNew = (level: number): Promise<GuideLevelModel> =>
		new Promise((resolve) => {
			...
			getMonsterListOfLevel({ level, bonus: currentLevelBonus }).then((currentLevelList) => {
	...
	generate = (): Promise<GuideResult> =>
		new Promise((resolve) => {
			...
			this.#taskGenerateLevelModelMappings().then(() => {
...
if (!worker.isMainThread) {
	worker.parentPort.on("message", (level) => {
		new LevelGuide(level).generate().then((res) => {
