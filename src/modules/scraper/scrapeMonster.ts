...
export const scrapeMonster = async (id: number) => {
	const ky = (await import("ky")).default;
	const { load } = await import("cheerio");
	const ID = id || 1;

	const html = await (
		await ky(`https://coryn.club/monster.php?id=${ID}`, {
			retry: {
				limit: 1e9,
			},
			timeout: 60e3,
	...
	$(".item-prop").each((_, elem) => {
		if ($(elem).html().includes("Spawn at")) {
			...
			monsterObject.map =
				mapEntry && mapEntry.text() !== "Event" ? `M${mapEntry.attr("href").match(/\d+/g).shift()}` : "Event";
		}
	});
