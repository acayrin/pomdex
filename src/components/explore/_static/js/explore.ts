...
function insertData(array: (ToramItem | ToramMap | ToramMonster)[]) {
	if (scriptData.explorer.innerHTML.includes("Empty")) return;

	if (array.length === 0) {
		const element = document.createElement("div");
		element.classList.add("col", "s12", "animate");

		const lines: string[] = [];
		lines.push("<div class='card hoverable'>");
		lines.push("<div class='card-content'>");
		lines.push(`<span class='card-title'>Empty</span>`);
		lines.push(`<b>Nothing was found or your query was incorrect.</b>`);
		lines.push("</div>");
		lines.push("</div>");
